class ServicesController < ApplicationController
  ...
  before_filter :authenticate_user!,
                :except => [:create, :signin, :signup, :newaccount, :failure]
  ...
  def create
    ...
    if omniauth and params[:service]
      ...
      if ["google", "yahoo", "twitter", "myopenid", "open_id"].index( service_route ) != nil
        omniauth['user_info']['email'] ?
            @authhash[:email] =  omniauth['user_info']['email'] :
            @authhash[:email] = ''
        omniauth['user_info']['name'] ?
            @authhash[:name] =  omniauth['user_info']['name'] :
            @authhash[:name] = ''
        omniauth['uid'] ?
      ...
      if @authhash[:uid] != '' and @authhash[:provider] != ''
        ...
        if user_signed_in?
          if auth
            ...
            current_user.services.create!(  :provider => @authhash[:provider],
                                            :uid => @authhash[:uid],
                                            :uname => @authhash[:name],
          ...
          if auth
            ...
            # this is a new user;
            # show signup;
            # @authhash is available to the view and stored in the sesssion for creation of a new user
